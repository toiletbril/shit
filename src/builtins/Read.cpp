...
i32 Read::execute(ExecContext &ec, EvalContext &cxt) const
{
  ...
  const usize operand_count = has_operands ? names.size() - first_operand : 1;
  ...
  const String line = String{StringView{*read_line}};

  const String field_separators =
