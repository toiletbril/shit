    ...
    throws -> metric_stats
{
  ...
  for (usize i = 0; i < samples.count(); i++) ordered.push(accessor(samples[i]));
...
/* One line of the summary table, the metric name, its mean and standard
   deviation, then its range, colored when allowed. */
fn append_metric_line(String &out, StringView name, const metric_stats &stats,
                      metric_unit unit, bool may_color) throws -> void
{
  out += "  ";
  out.append(name);
  for (usize pad = name.length; pad < 18; pad++) out.push(' ');

  out.append(colored(sgr::GREEN, may_color));
  out += format_metric(stats.mean, unit);
  out.append(colored(sgr::RESET, may_color));
  out += " +/- ";
  out.append(colored(sgr::GREEN, may_color));
  out += format_metric(stats.std_dev, unit);
  out.append(colored(sgr::RESET, may_color));

  out += "  (";
  out.append(colored(sgr::CYAN, may_color));
  out += format_metric(stats.min, unit);
  out.append(colored(sgr::RESET, may_color));
  out += " ... ";
  out.append(colored(sgr::MAGENTA, may_color));
  out += format_metric(stats.max, unit);
  out.append(colored(sgr::RESET, may_color));
  out += ")\n";
}
...
fn append_relative_line(String &out, StringView name,
                        const metric_stats &first, const metric_stats &other,
    ...
    throws -> command_result
{
  ...
  for (usize i = 0;; i++) {
    ...
    const bool reached_duration =
        !run_limit.has_value() &&
        (os::monotonic_nanos() - start_nanos) >= duration_nanos;
  ...
  result.wall_time =
      compute_stats(samples, [](const bench_sample &s) { return s.wall_nanos; });
  result.peak_rss = compute_stats(
      samples, [](const bench_sample &s) { return s.peak_rss_bytes; });
  result.cpu_cycles =
      compute_stats(samples, [](const bench_sample &s) { return s.cpu_cycles; });
    ...
    throws -> void
{
  ...
  append_metric_line(out, "wall time", result.wall_time,
                     metric_unit::Nanoseconds, may_color);
  append_metric_line(out, "peak rss", result.peak_rss, metric_unit::Bytes,
                     may_color);
  if (result.has_perf) {
    append_metric_line(out, "cpu cycles", result.cpu_cycles,
                       metric_unit::Count, may_color);
    append_metric_line(out, "instructions", result.instructions,
                       metric_unit::Count, may_color);
    append_metric_line(out, "cache refs", result.cache_references,
                       metric_unit::Count, may_color);
    append_metric_line(out, "cache misses", result.cache_misses,
                       metric_unit::Count, may_color);
    append_metric_line(out, "branch misses", result.branch_misses,
                       metric_unit::Count, may_color);
  }
}
