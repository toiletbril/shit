...
# of USR1 differs between platforms, so each section compares the reported status
# against the derived one. A child arrival leaves the wait blocked, because every
# external command reaps a child and a wake for each one would end every wait.
expected=$((128 + $(kill -l USR1)))
