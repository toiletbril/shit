...
# reference shell behaves the same way. Each whole listing is filtered to USR1,
# because the environment that starts the suite decides which other signals
# arrive ignored.
echo "== a signal ignored at entry lists with an empty action:"
( trap "" USR1
  "$BIN" --mood bash -c 'trap -p USR1; echo sep; trap -p | koshkit grep USR1' )
echo "rc=$?"
echo "== an action installed for that signal does not replace the listing:"
( trap "" USR1
  "$BIN" --mood bash -c \
    'trap "echo caught" USR1; trap -p USR1; echo sep
     trap -p | koshkit grep USR1' )
echo "rc=$?"
